...
namespace exams {
    ...
    public:
        ExamHall(X x, Y y, Grouping<Examinee> groupingFunctions) noexcept
                : examinees(x*y), x_size(x), y_size(y), groupingFunctions(groupingFunctions) {}
        ...
        //-------------------------------------------------------
        // try putting the following lines in comment
        //-------------------------------------------------------
        // Note:
        //-------------------------------------------------------
...
void bad_cases() {
    ...
    // bad - occupied sit
    try {
        std::cout << "expected occupied sit -- ";
    ...
    try {
        std::cout << "expected no examinee -- ";
        std::string examinee = examHall.unsit(X{1}, Y{1});
    } catch(BadPositionException& e) { /* no examinee at this location */ }

    // bad sit - wrong index
    try {
        std::cout << "expected bad index -- ";
        examHall.sit(X{1}, Y{8}, "Yossi");
    } catch(BadPositionException& e) { /* bad index Y {8} */ }
    std::cout << std::endl;
}
...
void test_copy_ctor_assignment_move() {
    if constexpr(!std::is_copy_constructible_v<ExamHall<std::string>>) {
        ...
        std::cout << "after test_copy_constructor" << std::endl;
    }
    if constexpr(!std::is_assignable_v<ExamHall<std::string>&, ExamHall<std::string>&>) {
        ...
        std::cout << "after test_assignment_operator" << std::endl;
    }
    if constexpr(!std::is_move_constructible_v<ExamHall<std::string>>) {
        ...
        std::cout << "after test_move_constructor" << std::endl;
    }
    if constexpr(!std::is_move_assignable_v<ExamHall<std::string>>) {
        ...
        std::cout << "after test_move_assignment_operator" << std::endl;
    }
}
